...
import tech.lapsa.esbd.domain.entities.ContactInfo;
import tech.lapsa.esbd.domain.entities.IdentityCardInfo;
...
public abstract class ASubjectEntityService<T extends SubjectEntity>
    ...
    List<T> _getByIdNumber(final TaxpayerNumber taxpayerNumber, final boolean fetchNaturals,
        ...
        try (Connection con = pool.getConnection()) {
            for (final int residentBool : residentBools)
                for (final int naturalPersonBool : naturalPersonBools) {
                    ...
                    resStream = Stream.concat(resStream, MyOptionals.of(clients) //
                            .map(ArrayOfClient::getClient).map(List::stream).orElseGet(Stream::empty));
                }
        } catch (ConnectionException e) {
            throw new IllegalStateException(e.getMessage());
        }
        return resStream.map(this::convert).collect(MyCollectors.unmodifiableList());
    }
    ...
    void fillValues(final Client source, final SubjectPersonEntityBuilder builder) {
        try {
...

            {
                ...
                PersonalInfo.builder()
                        ...
                        .withGender(Util.optField(SubjectPersonEntity.class, id, genders::getById, "personal.gender",
                                Sex.class, MyOptionals.of(source.getSexID())))
            ...
            IdentityCardInfo.builder() //
                    ...
                    .withIdentityCardType(Util.optField(SubjectPersonEntity.class, id, identityCardTypes::getById,
                            "identityCard.identityCardType", IdentityCardType.class,
    ...
    void fillValues(final Client source, final SubjectEntityBuilder<?, ?> builder) {
        try {
...

            {
                ...
                // WWW s:string Сайт
                ContactInfo.builder() //
                        .withPhone(MyOptionals.of(source.getPHONES()).map(PhoneNumber::assertValid)) //
                        .withHomeAdress(source.getAddress()) //
                        .withEmail(source.getEMAIL()) //
                        .withSiteUrl(source.getWWW()).buildTo(builder::withContact);
            }
...

            {
                ...
                builder.withIdNumber(MyOptionals.of(source.getIIN()).map(TaxpayerNumber::assertValid));
            }
